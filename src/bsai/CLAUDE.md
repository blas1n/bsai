...
```python
from bsai.events.bus import get_event_bus
...
All LLM prompts are managed as YAML templates in `src/bsai/prompts/`:
...
3. **Use in code**:
```python
from bsai.prompts import PromptManager, ArchitectPrompts
...
from bsai.api.dependencies import DBSession, CurrentUserId
