...
def _create_mock_task(session_id, user_id: str, **kwargs) -> MagicMock:
    """Create a mock task with session."""
    mock = MagicMock()
    mock.id = kwargs.get("id", uuid4())
    mock.session_id = session_id
    mock.session = MagicMock()
    mock.session.user_id = user_id
    mock.title = kwargs.get("title", "Test Task")
    mock.status = kwargs.get("status", "completed")
    return mock
...
class TestListArtifacts:
    """Test GET /sessions/{session_id}/tasks/{task_id}/artifacts endpoint."""
    ...
    def test_list_artifacts_success(
        ...
        mock_task = _create_mock_task(session_id, user_id, id=task_id)
        artifact1 = _create_mock_artifact(task_id, "main.py", "print('hello')", sequence_number=0)
        artifact2 = _create_mock_artifact(
            task_id, "utils.py", "def helper(): pass", sequence_number=1
        )
        ...
        db_session.execute = AsyncMock(side_effect=[mock_task_result, mock_artifacts_result])

        response = client.get(f"/sessions/{session_id}/tasks/{task_id}/artifacts")
    ...
    def test_list_artifacts_pagination(
        ...
        mock_task = _create_mock_task(session_id, user_id, id=task_id)
        artifacts = [
            _create_mock_artifact(task_id, f"file{i}.py", f"content{i}", sequence_number=i)
            for i in range(5)
        ]
        ...
        response = client.get(
            f"/sessions/{session_id}/tasks/{task_id}/artifacts",
    ...
    def test_list_artifacts_task_not_found(
        self,
        client: TestClient,
        db_session: AsyncMock,
    ):
    ...
    def test_list_artifacts_session_mismatch(
        self,
        client: TestClient,
        db_session: AsyncMock,
        user_id: str,
    ):
        """Test listing artifacts with wrong session ID."""
        session_id = uuid4()
        ...
        response = client.get(f"/sessions/{wrong_session_id}/tasks/{task_id}/artifacts")
    ...
    def test_list_artifacts_wrong_user(
        ...
        """Test listing artifacts for another user's task."""
        session_id = uuid4()
        task_id = uuid4()

        mock_task = _create_mock_task(session_id, "other-user", id=task_id)
...
class TestGetArtifact:
    """Test GET /sessions/{session_id}/tasks/{task_id}/artifacts/{artifact_id} endpoint."""
    ...
    def test_get_artifact_success(
        ...
        mock_task = _create_mock_task(session_id, user_id, id=task_id)
        artifact = _create_mock_artifact(task_id, "main.py", "print('hello')", id=artifact_id)
        ...
        db_session.execute = AsyncMock(side_effect=[mock_task_result, mock_artifact_result])

        response = client.get(f"/sessions/{session_id}/tasks/{task_id}/artifacts/{artifact_id}")
    ...
    def test_get_artifact_wrong_task(
        self,
        client: TestClient,
        db_session: AsyncMock,
        user_id: str,
    ):
        """Test getting artifact from wrong task."""
        session_id = uuid4()
        task_id = uuid4()
        other_task_id = uuid4()
        artifact_id = uuid4()
        ...
        mock_task = _create_mock_task(session_id, user_id, id=task_id)
        # Artifact belongs to different task
        artifact = _create_mock_artifact(other_task_id, "main.py", "content", id=artifact_id)
...
class TestDownloadArtifactsZip:
    """Test GET /sessions/{session_id}/tasks/{task_id}/artifacts/download/zip endpoint."""
    ...
    def test_download_zip_success(
        ...
        mock_task = _create_mock_task(session_id, user_id, id=task_id)
        artifact1 = _create_mock_artifact(task_id, "main.py", "print('hello')", path="src")
        artifact2 = _create_mock_artifact(
            task_id, "utils.py", "def helper(): pass", path="src/utils"
        )
        ...
        response = client.get(f"/sessions/{session_id}/tasks/{task_id}/artifacts/download/zip")
    ...
    def test_download_zip_no_path(
        ...
        mock_task = _create_mock_task(session_id, user_id, id=task_id)
        artifact = _create_mock_artifact(task_id, "main.py", "print('hello')", path="")
    ...
    def test_download_zip_duplicate_filenames(
        ...
        mock_task = _create_mock_task(session_id, user_id, id=task_id)
        artifact1 = _create_mock_artifact(task_id, "main.py", "content1", path="")
        artifact2 = _create_mock_artifact(task_id, "main.py", "content2", path="")
        ...
        with zipfile.ZipFile(zip_buffer, "r") as zf:
            file_list = zf.namelist()
            assert len(file_list) == 2
            # Should have renamed duplicate
            assert "main.py" in file_list
            assert "main_1.py" in file_list
    ...
    def test_download_zip_task_not_found(
        self,
        client: TestClient,
        db_session: AsyncMock,
    ):
        """Test downloading ZIP for non-existent task."""
        session_id = uuid4()
        task_id = uuid4()
    ...
    def test_download_zip_wrong_user(
        ...
        """Test downloading ZIP for another user's task."""
        session_id = uuid4()
        task_id = uuid4()
    ...
    def test_download_zip_path_with_filename(
        ...
        artifact = _create_mock_artifact(task_id, "main.py", "content", path="src/main.py")
    ...
    def test_download_zip_leading_slash_path(
        ...
        mock_task = _create_mock_task(session_id, user_id, id=task_id)
        artifact = _create_mock_artifact(task_id, "main.py", "content", path="/src")
